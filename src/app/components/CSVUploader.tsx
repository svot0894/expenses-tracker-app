...
export function CSVUploader({ onImportExpenses }: CSVUploaderProps) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Auto-detect columns
        const mapping = {
          title: headers.find(h => h.toLowerCase().includes('title') || h.toLowerCase().includes('description') || h.toLowerCase().includes('name')) || '',
  ...
  const handleImport = () => {
    if (!columnMapping.title || !columnMapping.amount || !columnMapping.category || !columnMapping.date) {
      setError('Please map all required columns');
      return;
    }

    try {
      const expenses: Omit<Expense, 'id'>[] = csvData.map(row => {
        let amount = parseFloat(row[columnMapping.amount]);
        // Handle amounts with dollar signs, commas, etc.
        if (isNaN(amount)) {
      ...
      }).filter(exp => exp.amount > 0); // Filter out invalid entries

      onImportExpenses(expenses);
      setIsOpen(false);
      setCSVData([]);
      setHeaders([]);
      setColumnMapping({ title: '', amount: '', category: '', date: '' });
      setError('');
    } catch (err) {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            CSV should contain columns for title, amount, category, and date
          </p>
        </div>
        ...
        {csvData.length > 0 && (
          <div className="flex gap-3">
            <button
              ...
              disabled={!columnMapping.title || !columnMapping.amount || !columnMapping.category || !columnMapping.date}
